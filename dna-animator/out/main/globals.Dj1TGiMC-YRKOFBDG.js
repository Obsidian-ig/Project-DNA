...
function diff(a, b2, options, memorize = DEFAULT_MEMORIZE) {
  ...
  if (expectedType !== getType3(b2)) {
    ...
    aDisplay = memorize("expected", truncate4(aDisplay));
    bDisplay = memorize("actual", truncate4(bDisplay));
...
function upperBound(haystack, needle, index3) {
  for (let i2 = index3 + 1; i2 < haystack.length; index3 = i2++) {
    if (haystack[i2][COLUMN] !== needle) break;
  }
  return index3;
}
function lowerBound(haystack, needle, index3) {
  for (let i2 = index3 - 1; i2 >= 0; index3 = i2--) {
    if (haystack[i2][COLUMN] !== needle) break;
  }
  return index3;
}
...
function traceSegmentInternal(segments, memo2, line, column, bias) {
  let index3 = memoizedBinarySearch(segments, column, memo2, line);
  if (found) {
...
function matchesTags(testTags) {
  ...
  const tagsFilter = runner2._currentSpecification?.testTagsFilter ?? runner2.config.tagsFilter;
  if (!tagsFilter) {
    return true;
  }
  let tagsFilterPredicate = filterMap.get(tagsFilter);
  if (!tagsFilterPredicate) {
    tagsFilterPredicate = createTagsFilter(tagsFilter, runner2.config.tags);
...
function tokenize(expr) {
  ...
  while (i2 < expr.length) {
    ...
    if (/^and(?:\s|\)|$)/i.test(expr.slice(i2))) {
      tokens.push({ type: "AND" });
      i2 += 3;
      continue;
    }
    if (/^or(?:\s|\)|$)/i.test(expr.slice(i2))) {
      tokens.push({ type: "OR" });
      i2 += 2;
      continue;
    }
    if (/^not\s/i.test(expr.slice(i2))) {
      tokens.push({ type: "NOT" });
      i2 += 3;
      continue;
    }
    let tag = "";
    while (i2 < expr.length && expr[i2] !== " " && expr[i2] !== "	" && expr[i2] !== "(" && expr[i2] !== ")" && expr[i2] !== "!" && expr[i2] !== "&" && expr[i2] !== "|") {
      const remaining = expr.slice(i2);
...
var TokenStream = class {
  ...
  expect(type3) {
    ...
    if (token.type !== type3) {
      if (type3 === "RPAREN" && token.type === "EOF") {
        throw new Error(`Invalid tags expression: missing closing ")" in "${this.expr}"`);
      }
      throw new Error(`Invalid tags expression: expected "${formatTokenType(type3)}" but got "${formatToken(token)}" in "${this.expr}"`);
    }
    return token;
  }
  unexpectedToken() {
...
function formatTokenType(type3) {
  switch (type3) {
    case "TAG":
      return "tag";
    case "AND":
...
function getTests(suite2) {
  const tests = [];
  const arraySuites = toArray(suite2);
...
function updateTask(event, task, runner2) {
  eventsPacks.push([
    task.id,
    event,
    void 0
  ...
  sendTasksUpdateThrottled(runner2);
}
...
async function saveInlineSnapshots(environment, snapshots) {
  const MagicString = (await import("./magic-string.es-H3BUBOAO.js")).default;
  ...
  await Promise.all(Array.from(files).map(async (file) => {
    ...
    for (const snap of snaps) {
      ...
      replaceInlineSnap(code, s, index3, snap.snapshot, snap.assertionName);
    }
...
function replaceObjectSnap(code, s, index3, newSnap, assertionName) {
...
function getCodeStartingAtIndex(code, index3, methodNames) {
  for (const name of methodNames) {
    const adjusted = index3 - name.length;
    if (adjusted >= 0 && code.slice(adjusted, index3) === name) {
      return {
...
function replaceInlineSnap(code, s, currentIndex, newSnap, assertionName) {
  ...
  const { code: codeStartingAtIndex, index: index3 } = getCodeStartingAtIndex(code, currentIndex, methodNames);
  ...
  if (!startMatch || startMatch.index !== firstKeywordMatch?.index) {
    return replaceObjectSnap(code, s, index3, newSnap, assertionName);
  }
      ...
      if (typeof _global.Promise !== "undefined") {
        clock.tickAsync = function tickAsync(tickValue) {
          return pauseAutoTickUntilFinished(
            new _global.Promise(function(resolve4, reject) {
...
function toMatchDomainSnapshotImpl(opts) {
  ...
  const pollFn = chai_exports.util.flag(assertion, "_poll.fn");
  if (pollFn) return getSnapshotClient().pollMatchDomain({
    poll: pollFn,
    adapter: opts.adapter,
    message: opts.hint,
    isInline: opts.isInline,
    errorMessage: chai_exports.util.flag(assertion, "message"),
    timeout: chai_exports.util.flag(assertion, "_poll.timeout"),
    interval: chai_exports.util.flag(assertion, "_poll.interval"),
    assertionName,
    inlineSnapshot,
...
async function runBenchmarkSuite(suite2, runner2) {
  const { Task, Bench } = await runner2.importTinybench();
  const start = performance.now();
  const benchmarkGroup = [];
  const benchmarkSuiteGroup = [];
  for (const task of suite2.tasks) {
  ...
  for (const subSuite of benchmarkSuiteGroup) await runBenchmarkSuite(subSuite, runner2);
  if (benchmarkGroup.length) {
    const defer = createDefer();
    suite2.result = {
      state: "run",
      startTime: start,
      benchmark: createBenchmarkResult(suite2.name)
    };
    updateTask$1("suite-prepare", suite2);
    const addBenchTaskListener = (task, benchmark) => {
      task.addEventListener("complete", (e) => {
        ...
        if (!runner2.config.benchmark?.includeSamples) result.samples.length = 0;
    ...
    for (const benchmark of benchmarkGroup) {
      ...
      await task.warmup();
      tasks.push([await new Promise((resolve4) => setTimeout3(async () => {
        resolve4(await task.run());
      })), benchmark]);
    }
    suite2.result.duration = performance.now() - start;
    suite2.result.state = "pass";
    updateTask$1("suite-finished", suite2);
    defer.resolve(null);
    await defer;
  }
  function updateTask$1(event, task) {
    updateTask(event, task, runner2);
  }
}
var NodeBenchmarkRunner = class {
  ...
  async importTinybench() {
    return await import("./dist-OIU7R5WM.js");
  }
  ...
  async runSuite(suite2) {
    await runBenchmarkSuite(suite2, this);
  }
  async runTask() {
...
var TestRunner = class {
  ...
  constructor(config2) {
    ...
    this.viteModuleRunner = config2.experimental.viteModuleRunner;
  }
  importFile(filepath, source) {
    if (source === "setup") {
  ...
  async onAfterRunSuite(suite2) {
    if (this.config.logHeapUsage && typeof process !== "undefined") suite2.result.heap = process.memoryUsage().heapUsed;
    if (suite2.mode !== "skip" && "filepath" in suite2) {
      for (const test5 of getTests(suite2)) if (test5.mode === "skip") {
        const name = getNames(test5).slice(1).join(" > ");
        this.snapshotClient.skipTest(suite2.file.filepath, name);
      }
      const result = await this.snapshotClient.finish(suite2.file.filepath);
...
`;
        for (const key of result.uncheckedKeys) message += `\xB7 ${key}
`;
        suite2.result.errors ??= [];
        suite2.result.errors.push(processError(new Error(message)));
        suite2.result.state = "fail";
      }
      await rpc().snapshotSaved(result);
    }
    this.workerState.current = suite2.suite || suite2.file;
  }
  onAfterRunTask(test5) {
    if (this.config.logHeapUsage && typeof process !== "undefined") test5.result.heap = process.memoryUsage().heapUsed;
    this.workerState.current = test5.suite || test5.file;
  }
  cancel(_reason) {
    this.cancelRun = true;
  }
  ...
  async onBeforeRunTask(test5) {
    if (this.cancelRun) test5.mode = "skip";
    if (test5.mode !== "run" && test5.mode !== "queued") return;
    this.workerState.current = test5;
  }
  async onBeforeRunSuite(suite2) {
    if (this.cancelRun) suite2.mode = "skip";
    if (suite2.mode !== "skip" && "filepath" in suite2) await this.snapshotClient.setup(suite2.file.filepath, this.workerState.config.snapshotOptions);
    this.workerState.current = suite2;
  }
  onBeforeTryTask(test5) {
    clearModuleMocks(this.config);
    this.snapshotClient.clearTest(test5.file.filepath, test5.id);
    setState({
      assertionCalls: 0,
      isExpectingAssertions: false,
      isExpectingAssertionsError: null,
      expectedAssertionsNumber: null,
      expectedAssertionsNumberErrorGen: null,
      currentTestName: getTestName(test5),
      snapshotState: this.snapshotClient.getSnapshotState(test5.file.filepath)
    }, globalThis[GLOBAL_EXPECT]);
  }
  onAfterTryTask(test5) {
    const { assertionCalls, expectedAssertionsNumber, expectedAssertionsNumberErrorGen, isExpectingAssertions, isExpectingAssertionsError } = test5.context._local ? test5.context.expect.getState() : getState(globalThis[GLOBAL_EXPECT]);
    ...
    if (this.config.expect.requireAssertions && assertionCalls === 0) throw this.assertionsErrors.get(test5);
  }
  ...
  getImportDurations() {
    ...
    if (limit === 0) return {};
    const sortedEntries = [...this.workerState.moduleExecutionInfo?.entries() || []].sort(([, a], [, b2]) => b2.duration - a.duration).slice(0, limit);
...
function clearModuleMocks(config2) {
  const { clearMocks, mockReset, restoreMocks, unstubEnvs, unstubGlobals } = config2;
  ...
  */
  ensureModule(id, url) {
    if (id = normalizeModuleId(id), this.idToModuleMap.has(id)) {
      let moduleNode2 = this.idToModuleMap.get(id);
